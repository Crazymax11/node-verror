...
function VError()
{
	...
	if (cause) {
		mod_assertplus.ok(isError(cause), 'cause is not an Error');
...
{
	mod_assertplus.ok(isError(err), 'err must be an Error');
...
VError.findCauseByName = function (err, name)
{
	...
	for (cause = err; cause !== null; cause = VError.cause(cause)) {
		mod_assertplus.ok(isError(cause));
		if (cause.name == name) {
			return (cause);
		}
	}
...
{
	if (!Array.isArray(errors) || !errors.every(err => typeof err === 'object' && err != null)) {
		throw new Error('AssertionError: errors ([object]) is required');
	}
